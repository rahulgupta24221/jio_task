const bookitem = require('../model/book');
...
async function addtask (req, res) {
 ...
 async function updatetask (req, res)  {
    ...
    try {
        let up_todo = await bookitem.findByIdAndUpdate(todoid, { title, author,published });
        if (!up_todo) {
 ...
 async function del_task (req, res) {
    ...
    try {
        ...
        let del_item = await bookitem.findByIdAndDelete(id).populate("user");
        // it is deleting a item in todo of user 
        await del_item.user.book.pull(del_item);
